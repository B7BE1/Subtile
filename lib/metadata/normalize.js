 ...
 *   sources
 * }
 * (filler/recap are anime-only, populated when Jikan episode detail data
 * was available; absent/false for Cinemeta-sourced records.)
 */
 ...
 * @param {Array<{episode: number, title: string|null, title_japanese: string|null, aired: string|null, filler: boolean, recap: boolean}>} [episodeDetails]
 *   Optional real per-episode data from fetchJikanEpisodes — when omitted,
 *   episodes fall back to numbered placeholders as before.
 * @returns {object} normalized record
 */
export function normalizeJikan(a, episodeDetails = []) {
  ...
  const detailsByNumber = new Map(episodeDetails.map((e) => [e.episode, e]));
  ...
  if (a.type === 'Movie') {
    ...
    // Real titles/air-dates/filler-recap flags come from episodeDetails
    // (fetchJikanEpisodes) when available; falls back to a numbered
    // placeholder for any episode Jikan hasn't detailed yet (e.g. unaired).
    episodes = Array.from({ length: episodeCount || 12 }, (_, i) => {
      const num = i + 1;
      const detail = detailsByNumber.get(num);
      return {
        id: null,
        season: 1,
        episode: num,
        episode_absolute: num,
        title: detail?.title || null,
        released: detail?.aired || null,
...
/**
 * Lightweight search-result normalizer (Cinemeta catalog entry, not a full
 * meta lookup) — kept field-consistent with normalizeCinemeta so search
 * results and detail-page results never disagree on shape.
 * @param {object} m raw Cinemeta catalog meta object
 * @param {"movie"|"tv"} type
 * @returns {object}
 */
export function normalizeCinemetaSearchResult(m, type) {
  const poster = m.poster || '';
  return {
    id: m.imdb_id || m.id,
    type,
    imdb_id: m.imdb_id || m.id || null,
    mal_id: null,
    title: m.name || '',
    year: parseInt(m.year, 10) || null,
    rating: parseFloat(m.imdbRating) || 0,
    poster,
    backdrop: m.background || poster,
    genres: m.genres || [],
    sources: ['cinemeta'],
  };
}
...
/**
 * Lightweight search-result normalizer (Jikan search entry, not the /full
 * lookup) — kept field-consistent with normalizeJikan.
 * @param {object} a raw Jikan search-result anime object
 * @returns {object}
 */
export function normalizeJikanSearchResult(a) {
  const poster = a.images?.webp?.large_image_url || a.images?.jpg?.large_image_url || '';
  return {
    id: `anime-${a.mal_id}`,
    type: 'anime',
